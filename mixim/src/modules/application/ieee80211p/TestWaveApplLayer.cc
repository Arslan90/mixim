...
void TestWaveApplLayer::onBeacon(WaveShortMessage* wsm) {
	...
	DBG << "In TestWaveAppl: Received beacon priority  " << wsm->getPriority() << " at " << simTime() << std::endl;
...
void TestWaveApplLayer::onData(WaveShortMessage* wsm) {
	...
	if (recipientId == myId) {
		DBG  << "In TestWaveAppl: Received data priority  " << wsm->getPriority() << " at " << simTime() << std::endl;
		receivedData++;
	}
}
