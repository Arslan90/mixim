...
std::string TraCIScenarioManager::receiveTraCIMessage() {
	...
	char buf[bufLength];
	{
		//Arturo: I disable this echo just to avoid to much verbose
...
//ARTURO
//Voici mes functions pour faire les requetes des que je besion.
std::list<std::string> TraCIScenarioManager::commandGetVehiclesIds() {
	std::list<std::string> res;

	std::string objectId = "";
	TraCIBuffer buf = queryTraCI(CMD_GET_VEHICLE_VARIABLE, TraCIBuffer() << static_cast<uint8_t>(ID_LIST) << objectId);
...
std::list<std::string> TraCIScenarioManager::commandGetArcIds() {
	std::list<std::string> res;

	std::string objectId = "";
	TraCIBuffer buf = queryTraCI(CMD_GET_LANE_VARIABLE, TraCIBuffer() << static_cast<uint8_t>(ID_LIST) << objectId);

	// read additional CMD_GET_LANE_VARIABLE sent back in response
	uint8_t cmdLength; buf >> cmdLength;
	if (cmdLength == 0) {
		uint32_t cmdLengthX;
		buf >> cmdLengthX;
...
std::list<std::string> TraCIScenarioManager::commandGetSingleVehicleRoutes(std::string nodeId) {
	std::list<std::string> res;

	std::string objectId = "";
	uint8_t variableType = TYPE_STRINGLIST;
	//Variable 	VehicleTypeID
	TraCIBuffer buf = queryTraCI(CMD_GET_VEHICLE_VARIABLE, TraCIBuffer() << static_cast<uint8_t>(VAR_EDGES) << nodeId );
...
double TraCIScenarioManager::commandGetArcCO2(std::string edgeId) {
	...
	TraCIBuffer buf = queryTraCI(CMD_GET_EDGE_VARIABLE, TraCIBuffer() << static_cast<uint8_t>(VAR_CO2EMISSION) << edgeId);
...
double TraCIScenarioManager::commandGetArcOccupancy(std::string edgeId) {
	//std::string res;
	double res;

	TraCIBuffer buf = queryTraCI(CMD_GET_EDGE_VARIABLE, TraCIBuffer() << static_cast<uint8_t>(LAST_STEP_OCCUPANCY) << edgeId);
	...
	uint8_t varId; buf >> varId; //ubyte
	ASSERT(varId == LAST_STEP_OCCUPANCY);
...
double TraCIScenarioManager::cmdGetVehiclelanePosition(std::string nodeId) {
	double res;

	TraCIBuffer buf = queryTraCI(CMD_GET_VEHICLE_VARIABLE, TraCIBuffer() << static_cast<uint8_t>(VAR_LANEPOSITION) << nodeId);
...
double TraCIScenarioManager::cmdGetLaneLength(std::string laneId) {
	double res;

	TraCIBuffer buf = queryTraCI(CMD_GET_LANE_VARIABLE, TraCIBuffer() << static_cast<uint8_t>(VAR_LENGTH) << laneId);
...
void TraCIScenarioManager::executeOneTimestep() {
	...
	if (targetTime > 0) {
		...
		uint32_t count; buf >> count;
		MYDEBUG << "Getting " << count << " subscription results" <<" at "<< simTime()<< endl;
		for (uint32_t i = 0; i < count; ++i) {
			processSubcriptionResult(buf);
		}
	}
...
void TraCIScenarioManager::processSimSubscription(std::string objectId, TraCIBuffer& buf) {
	...
	for (uint8_t j = 0; j < variableNumber_resp; ++j) {
		...
		if (variable1_resp == VAR_DEPARTED_VEHICLES_IDS) {
			...
			uint32_t count; buf >> count;
			MYDEBUG << "TraCI reports " << count << " departed vehicles." <<" at "<< simTime()<< endl;
			for (uint32_t i = 0; i < count; ++i) {
			...
			ASSERT(varType == TYPE_STRINGLIST);
			uint32_t count; buf >> count;
			MYDEBUG << "TraCI reports " << count << " arrived vehicles." <<" at "<< simTime()<< endl;
			for (uint32_t i = 0; i < count; ++i) {
				std::string idstring; buf >> idstring;
...
void TraCIScenarioManager::processVehicleSubscription(std::string objectId, TraCIBuffer& buf) {
	...
	for (uint8_t j = 0; j < variableNumber_resp; ++j) {
		...
		if (isokay != RTYPE_OK) {
			...
			ASSERT(varType == TYPE_STRINGLIST);
			uint32_t count; buf >> count;
			//Arturo. I need this data so I'll add "logs" in order to reconize it in logs...
			MYDEBUG << "logs, TraCI reports " << count << " active vehicles." <<" at "<< simTime()<< endl;
