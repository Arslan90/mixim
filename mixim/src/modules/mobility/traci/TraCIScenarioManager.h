...
class TraCIScenarioManager : public cSimpleModule
{
	public:
		...
		std::list<std::string> commandGetSingleVehicleRoutes(std::string nodeId);  //TESTING..
        double cmdGetVehiclelanePosition(std::string polyId);
        ...
        void commandChangeRouteByListOfEdges(std::string nodeId, int nbrEdge, std::list<std::string> listOfEdges);
...
public:
    ...
    simtime_t getUpdateInterval() const;
};
