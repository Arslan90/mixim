...
#include "mobility/simpleTraci/simpleTraCIMobility.h"

Define_Module(simpleTraCIMobility);
...
void simpleTraCIMobility::Statistics::initialize()
{
...
void simpleTraCIMobility::Statistics::watch(cSimpleModule& )
{
	WATCH(totalTime);
	WATCH(minSpeed);
	WATCH(maxSpeed);
...
void simpleTraCIMobility::Statistics::recordScalars(cSimpleModule& module)
{
...
void simpleTraCIMobility::initialize(int stage)
{
	if (stage == 0)
	{
	...
	else if (stage == 1)
	{
		...
		if (stopPos != -1 ){
			if (stopDuration <= 0){
				opp_warning("Stop duration is equal or less than zero");
			}
			if ((stopDistStep != -1) && (releasePos != -1)){
				double posAt127M = targetPos - 127;
				for (double i = stopPos;i <= releasePos; i+=stopDistStep){
					if ((with127M) && ((i <= posAt127M) && (posAt127M <= i+stopDistStep))){
						commandStopNode(getRoadId(),i,0,0,stopDuration);
						commandStopNode(getRoadId(),posAt127M,0,0,stopDuration);
					}else {
						commandStopNode(getRoadId(),i,0,0,stopDuration);
					}
				}
			}else {
				commandStopNode(getRoadId(),stopPos,0,0,stopDuration);
			}
		}
	}
	else
...
void simpleTraCIMobility::finish()
{
...
void simpleTraCIMobility::handleSelfMsg(cMessage *msg)
{
...
void simpleTraCIMobility::preInitialize(std::string external_id, const Coord& position, std::string road_id, double speed, double angle)
{
...
void simpleTraCIMobility::nextPosition(const Coord& position, std::string road_id, double speed, double angle)
{
...
void simpleTraCIMobility::changePosition()
{
	...
	move.setDirectionByVector(Coord(cos(angle), -sin(angle)));
	move.setSpeed(speed);
//	if ((speed == 0) && !stoppedVeh){
	if ((speed == 0) && !stoppedVeh){
		stoppedVeh = true;
		double lanePos = cmdGetVehiclelanePosition();
...
void simpleTraCIMobility::updateDisplayString() {
	ASSERT(-M_PI <= angle);
	ASSERT(angle < M_PI);
...
void simpleTraCIMobility::fixIfHostGetsOutside()
{
...
void simpleTraCIMobility::simulateAccident()
{
...
void simpleTraCIMobility::receiveSignal(cComponent *source, simsignal_t signalID, long  l)
{
}

void simpleTraCIMobility::receiveSignal(cComponent *source, simsignal_t signalID, double d)
{
}

double simpleTraCIMobility::calculateCO2emission(double v, double a) const {
...
double simpleTraCIMobility::getReleasePos() const
{
    return releasePos;
}

double simpleTraCIMobility::getStopDistStep() const
{
    return stopDistStep;
}

double simpleTraCIMobility::getStopDuration() const
{
    return stopDuration;
}

double simpleTraCIMobility::getStopPos() const
{
    return stopPos;
}

void simpleTraCIMobility::setReleasePos(double releasePos)
{
    this->releasePos = releasePos;
}

void simpleTraCIMobility::setStopDistStep(double stopDistStep)
{
    this->stopDistStep = stopDistStep;
}

void simpleTraCIMobility::setStopDuration(double stopDuration)
{
    this->stopDuration = stopDuration;
}

void simpleTraCIMobility::setStopPos(double stopPos)
{
