...
Define_Module(simpleProphet);

void simpleProphet::initialize(int stage)
{
	...
	if (stage==0){
		...
		simpleTraCIMobility* mobility = FindModule<simpleTraCIMobility*>::findSubModule(this->getParentModule());
    	if (mobility != NULL){
...
void simpleProphet::updateDeliveryPredsFor(const LAddress::L3Type BAdress)
{
	if (BAdress == myNetwAddr){
		opp_warning("Cannot Update Delivery Predictions for its own Address(simpleProphet::updateDeliveryPredsFor)");
	}else{
...
void simpleProphet::updateTransitivePreds(const LAddress::L3Type BAdress, std::map<LAddress::L3Type,double> Bpreds)
{
...
void simpleProphet::ageDeliveryPreds()
{
	predsIterator it2;
...
void simpleProphet::update(Prophet *prophetPkt)
{
...
void simpleProphet::partialUpdate(Prophet *prophetPkt)
{
	updateTransitivePreds(prophetPkt->getSrcAddr(),prophetPkt->getPreds());
	recordPredsStats();
}
...
void simpleProphet::handleLowerMsg(cMessage* msg)
{
...
void simpleProphet::handleLowerControl(cMessage* msg)
{
...
void simpleProphet::handleSelfMsg(cMessage* msg)
{
...
void simpleProphet::storeBundle(WaveShortMessage *msg)
{
...
void simpleProphet::executeInitiatorRole(short  kind, Prophet *prophetPkt)
{
	/*
	...
	if (recordContactStats){
		...
		if (iterator1 == indexContactStats.end()){
			 ...
			 */
			opp_error("looking for an non-existent contact(simpleProphet::executeInitiatorRole)");
		}else {
			...
			if (abortConnection(Bundle_Offer,prophetPkt)){
				...
				for (std::list<BundleMeta>::iterator it = prophetPkt->getBndlmeta().begin(); it !=prophetPkt->getBndlmeta().end(); ++it) {
					if (it->getFlags() == Prophet_Enum::Bndl_Accepted){
						...
						opp_error("Reception of bundle Meta of unknown type(simpleProphet::executeInitiatorRole)");
					}
...
void simpleProphet::executeListenerRole(short  kind, Prophet *prophetPkt)
{
	/*
	 * First we check if we actually record stats about contact
	 */
		...
		if (iterator1 == indexContactStats.end()){
			if (kind == RIB){
				...
				if (iterator2 == indexContactStats.end()){
					 ...
					 * No entry found for this address, it is impossible
					 */
					opp_error("looking for an non-existent contact that has been inserted(simpleProphet::executeInitiatorRole)");
				}else{
				...
				for (unsigned int i  = 0; i  < allBundleMeta.size(); i++ ) {
					...
					switch (i) {
						...
						case 2:
							...
							break;
						default:
							opp_error("definition of Bundle Offer must return a vector of size equal to 3(simpleProphet::executeListenerRole)");
							break;
					}
...
Prophet *simpleProphet::prepareProphet(short  kind, LAddress::L3Type srcAddr,LAddress::L3Type destAddr, std::list<BundleMeta> *meta, std::map<LAddress::L3Type,double> *preds, WaveShortMessage *msg)
{
...
std::vector<std::list<BundleMeta> >simpleProphet::defineBundleOffer(Prophet *prophetPkt)
{
	...
	if (allBundleMeta.size()!=3){
		opp_error("definition of Bundle Offer must return a vector of size equal to 3(simpleProphet::defineBundleOffer)");
	}

	return allBundleMeta;
}

bool simpleProphet::abortConnection(short  kind, Prophet *prophetPkt)
{
	bool abort = false;
	...
	switch (kind) {
		...
		case Bundle_Ack:
			break;
		default:
			opp_error("Unknown Prophetv2MessageKinds(simpleProphet::abortConnection)");
			break;
	}
...
void simpleProphet::updateContactWhenInit(Prophet *prophetPkt, unsigned long contactID, SimpleContactStats contact, int kind)
{
	/*
...
void simpleProphet::updateContactWhenList(Prophet *prophetPkt, unsigned long contactID, SimpleContactStats contact, int kind)
{
	/*
	 * Last we update stats about contact
	 */
	if (recordContactStats){
		...
		if (iterator3 == indexContactID.end()){
			 ...
			 * No entry found for this address, it is impossible
			 */
			opp_error("looking for an non-existent contact(simpleProphet::executeListenerRole)");
		}else {
			if (iterator3->second.size()>1){
				contact.setRepeatedContact(true);
			}
...
//	EV << "Received: " << nbrL3Received << endl;
	...
	for (std::map<std::string,int>::iterator it = nbrSentByDist.begin(); it != nbrSentByDist.end(); it++) {
		std::string tmp = "# nbrSent_"+it->first;
...
void simpleProphet::recordPredsStats()
{
...
void simpleProphet::recordEndContactStats(LAddress::L3Type addr, double time)
{
...
void simpleProphet::recordRecontactStats(LAddress::L3Type addr, double time)
{
...
void simpleProphet::updatingContactState(LAddress::L3Type addr, Prophetv2MessageKinds kind)
{
	std::map<LAddress::L3Type, Prophetv2MessageKinds>::iterator it = contactState.find(addr);
	if (it != contactState.end()){
		contactState.erase(addr);
...
void simpleProphet::classify(SimpleContactStats* newContact)
{
	LEG_DtnNetwLayer::classify(newContact);
...
void simpleProphet::recordAllClassifier()
{
...
void simpleProphet::initAllClassifier()
{
...
void simpleProphet::updatingL3Sent()
{
	LEG_DtnNetwLayer::updatingL3Sent();
    std::map<std::string,int>::iterator it = nbrSentByDist.find(dist);
    if (it == nbrSentByDist.end()){
    	nbrSentByDist.insert(std::pair<std::string,double>(dist,1));
    }else{
    	nbrSentByDist[dist] = it->second + 1;
    }
}

void simpleProphet::updatingL3Received()
{
	LEG_DtnNetwLayer::updatingL3Received();
...
simpleProphet::~simpleProphet()
{

}

void simpleProphet::receiveSignal(cComponent *source, simsignal_t signalID, long  l)
{
	Enter_Method_Silent("simpleProphet::receiveSignal()");
...
void simpleProphet::receiveSignal(cComponent *source, simsignal_t signalID, double d)
{
	Enter_Method_Silent("simpleProphet::receiveSignal()");
