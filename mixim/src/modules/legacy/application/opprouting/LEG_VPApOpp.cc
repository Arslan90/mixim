...
#include "LEG_VPApOpp.h"
#include "multiFunctions.h"
//#include "ApplOppControlInfo.h"
#include "DtnNetwLayer.h"
#include "clistener.h"

Define_Module(LEG_VPApOpp);

void LEG_VPApOpp::initialize(int stage) {
	...
	if (stage==0) {
    	...
    	if (updateSectorCycle >=0){
    		...
    		opp_error("updateSectorCycle value cannot be negative(LEG_VPApOpp::initialize)");
    	}
    	currentVehDensity = 0;
...
void LEG_VPApOpp::handleSelfMsg(cMessage* msg) {
...
//	    		vVehMeanBndlSuccSent.record(meanBndlSuccSent);
	    		...
	    		opp_error("updateSectorCycle value cannot be negative(LEG_VPApOpp::handleSelfMsg)");
	    	}
			break;
		}
		default: {
...
void LEG_VPApOpp::handleLowerMsg(cMessage* msg) {
...
//			vehicleDensity.record(currentVehDensity);
			...
			if (nbrUniqueBundleReceived>0){
				...
				opp_error("Nbr Unique Bundle Received equal or less then zero(LEG_VPApOpp::handleLowerMsg)");
			}
...
void LEG_VPApOpp::sendVPApBroadcast(int messageSequence) {
...
WaveShortMessage*  LEG_VPApOpp::prepareWSM(std::string name, int lengthBits, t_channel channel, int priority, int rcvId, unsigned long serial) {
...
bool LEG_VPApOpp::bundleExistUnderOtherVPA(unsigned long  serial)
{
	bool exist = false;
	...
	for (int i = 0; i < numberVPA; ++i) {
		...
		if (vpa!=NULL){
			...
			if (appl!=NULL){
				LEG_VPApOpp *vpa_appl = check_and_cast<LEG_VPApOpp*>(appl);
...
void LEG_VPApOpp::sendDtnMessage()
{
...
void LEG_VPApOpp::receiveSignal(cComponent *source, simsignal_t signalID, const char *s)
{
	Enter_Method_Silent();
...
int LEG_VPApOpp::randomVPADestAddr()
{
	int vpaDestAddr = -2;
	cModule *systemModule = this->getParentModule();
	while (systemModule->getParentModule() !=NULL){
...
void LEG_VPApOpp::onBeacon(WaveShortMessage* wsm) {
}

void LEG_VPApOpp::onData(WaveShortMessage* wsm) {
}

LEG_VPApOpp::~LEG_VPApOpp() {
}

void LEG_VPApOpp::finish()
{
	DtnApplLayer::finish();
...
void LEG_VPApOpp::resetL20Stats()
{
	total0RcvH = 0;
	total0RcvB = 0;
	total0RcvA = 0;
...
void LEG_VPApOpp::updateL20Stats(long  rcvH, long  rcvB, long  rcvA, long  L2S, long  L2D, long  L2R, long  L2L, long  L2Back, long  L2BackDur)
{
	total0RcvH += rcvH;
	total0RcvB += rcvB;
	total0RcvA += rcvA;
...
void LEG_VPApOpp::resetL21Stats()
{
	total1RcvH = 0;
	total1RcvB = 0;
	total1RcvA = 0;
...
void LEG_VPApOpp::updateL21Stats(long  rcvH, long  rcvB, long  rcvA, long  L2S, long  L2D, long  L2R, long  L2L, long  L2Back, long  L2BackDur)
{
	total1RcvH += rcvH;
	total1RcvB += rcvB;
	total1RcvA += rcvA;
...
void LEG_VPApOpp::resetL22Stats()
{
	total2RcvH = 0;
	total2RcvB = 0;
	total2RcvA = 0;
...
void LEG_VPApOpp::updateL22Stats(long  rcvH, long  rcvB, long  rcvA, long  L2S, long  L2D, long  L2R, long  L2L, long  L2Back, long  L2BackDur)
{
	total2RcvH += rcvH;
	total2RcvB += rcvB;
	total2RcvA += rcvA;
...
void LEG_VPApOpp::resetL23Stats()
{
	total3RcvH = 0;
	total3RcvB = 0;
	total3RcvA = 0;
...
void LEG_VPApOpp::updateL23Stats(long  rcvH, long  rcvB, long  rcvA, long  L2S, long  L2D, long  L2R, long  L2L, long  L2Back, long  L2BackDur)
{
	total3RcvH += rcvH;
	total3RcvB += rcvB;
	total3RcvA += rcvA;
...
void LEG_VPApOpp::recordL2Stats()
{
