...
//#include <unistd.h>
#include "LEG_myConstants.h"
#include "errno.h"
#include "LEG_DtnNetwLayer.h"
#include "FindModule.h"
#include "Mac80211p.h"

#include "legacy/mobility/geoTraCI/LEG_GeoTraCIMobility.h"


Define_Module(LEG_GeoTraCIMobility);
...
void LEG_GeoTraCIMobility::Statistics::initialize()
{
...
void LEG_GeoTraCIMobility::Statistics::watch(cSimpleModule& )
{
	WATCH(totalTime);
	WATCH(minSpeed);
	WATCH(maxSpeed);
...
double LEG_GeoTraCIMobility::getCurrentMetd() const
{
    return currentMETD;
}

NearestPoint LEG_GeoTraCIMobility::getCurrentNp() const
{
    return currentNP;
}

void LEG_GeoTraCIMobility::setCurrentMetd(double currentMetd)
{
    currentMETD = currentMetd;
}

void LEG_GeoTraCIMobility::Statistics::recordScalars(cSimpleModule& module)
{
...
void LEG_GeoTraCIMobility::initialize(int stage)
{
	if (stage == 0)
	{
...
void LEG_GeoTraCIMobility::finish()
{
...
void LEG_GeoTraCIMobility::handleSelfMsg(cMessage *msg)
{
...
void LEG_GeoTraCIMobility::preInitialize(std::string external_id, const Coord& position, std::string road_id, double speed, double angle)
{
...
void LEG_GeoTraCIMobility::nextPosition(const Coord& position, std::string road_id, double speed, double angle)
{
...
void LEG_GeoTraCIMobility::changePosition()
{
...
void LEG_GeoTraCIMobility::updateCurrentSector()
{
	if (scenarioModel == Sector){
		...
		if (inSector != currentSector){
			...
			if (getParentModule()->findSubmodule("netw")!=-1){
				LEG_DtnNetwLayer* netwMod = FindModule<LEG_DtnNetwLayer*>::findSubModule(getParentModule());
...
void LEG_GeoTraCIMobility::reComputeMETDAfterReRoute(){
...
int LEG_GeoTraCIMobility::roadIndexInRoute(std::string roadId, std::list<std::string> route, int indexLastRoadId, bool canEquals)
{
	bool found = false;
	int index = -1, i = -1;
...
bool LEG_GeoTraCIMobility::roadInRoute(std::string roadId, std::list<std::string> route)
{
	bool found = false;
	if (!route.empty()){
		for (std::list<std::string>::iterator it = route.begin(); it != route.end(); it++){
...
void LEG_GeoTraCIMobility::updateRemainingRoute()
{
...
void LEG_GeoTraCIMobility::updateMETD(bool reComputeETA_NP_VPA)
{
...
double LEG_GeoTraCIMobility::calculateRmgRoadBestTT()
{
...
double LEG_GeoTraCIMobility::calculateETA_NP(bool includeCurrentRoad)
{
...
double LEG_GeoTraCIMobility::calculateETA_NP_VPA()
{
...
double LEG_GeoTraCIMobility::getEdgeBestTravelTime(std::string edgeId)
{
	...
	if (edgesBTTIndex.find(edgeId) != edgesBTTIndex.end()){
		...
		int msg_type = LEG_MY_CONST::QUERY;
		int cmd = LEG_MY_CONST::CMD_EDGE_BEST_TRAVEL_TIME;

		int arg1 = LEG_MY_CONST::EDGE_ID;
		std::string arg1_value = edgeId;

		std::string msg = "";
		msg+= LEG_MY_CONST::convertToStr(msg_type)+":"+LEG_MY_CONST::convertToStr(cmd)+";";
		msg+= LEG_MY_CONST::convertToStr(arg1)+":"+arg1_value+"?";

		// Sending request to server
		std::string queryRep = sendRequestToPyServer(msg);

		// Tokenizing request result
		std::vector<std::string> reponse_tokens = LEG_MY_CONST::tokenizeMSG(queryRep);
		...
		bool abort = false;
		if (reponse_tokens[0] != LEG_MY_CONST::convertToStr(LEG_MY_CONST::RESPONSE)) {abort = true;}
		if (reponse_tokens[1] != LEG_MY_CONST::convertToStr(LEG_MY_CONST::RESPONSE_EDGE_BEST_TRAVEL_TIME)) {abort = true;}
		if (reponse_tokens[2] != LEG_MY_CONST::convertToStr(LEG_MY_CONST::EDGE_BEST_TRAVEL_TIME)) {abort = true;}

		if (abort){
			opp_warning("Bad format for reponse_EDGE_BEST_TRAVEL_TIME");
		}else{
			travelTime = LEG_MY_CONST::convertToDbl(reponse_tokens[3]);
		}
	}
...
NearestPoint LEG_GeoTraCIMobility::getNearestPoint(int vpaSectorId, std::list<std::string> currentRoute)
{
	NearestPoint returnedNP = NearestPoint();
	int msg_type = LEG_MY_CONST::QUERY;
	int cmd = LEG_MY_CONST::CMD_NP_ALL;

	int arg1 = LEG_MY_CONST::VPA_SECTOR_ID;
	int arg1_value = vpaSectorId;

	int arg2 = LEG_MY_CONST::ROUTE_CURRENT;
	...
	int arg3 = LEG_MY_CONST::HOTSPOT_MODE;
	...
	std::string msg = "";
	msg+= LEG_MY_CONST::convertToStr(msg_type)+":"+LEG_MY_CONST::convertToStr(cmd)+";";
	msg+= LEG_MY_CONST::convertToStr(arg1)+":"+LEG_MY_CONST::convertToStr(arg1_value)+";";
	msg+= LEG_MY_CONST::convertToStr(arg2)+":"+arg2_value+";";
	msg+= LEG_MY_CONST::convertToStr(arg3)+":"+arg3_value+"?";
	...
	bool abort = false;
	if (reponse_tokens[0] != LEG_MY_CONST::convertToStr(LEG_MY_CONST::RESPONSE)) {abort = true;}
	if (reponse_tokens[1] != LEG_MY_CONST::convertToStr(LEG_MY_CONST::RESPONSE_NP_ALL)) {abort = true;}
	if (reponse_tokens[2] != LEG_MY_CONST::convertToStr(LEG_MY_CONST::EDGE_TO_NP)) {abort = true;}
	if (reponse_tokens[4] != LEG_MY_CONST::convertToStr(LEG_MY_CONST::NODE_NP)) {abort = true;}
	if (reponse_tokens[6] != LEG_MY_CONST::convertToStr(LEG_MY_CONST::EDGE_FROM_NP)) {abort = true;}
	if (reponse_tokens[8] != LEG_MY_CONST::convertToStr(LEG_MY_CONST::NODE_VPA_MAPPING)) {abort = true;}
	if (reponse_tokens[10] != LEG_MY_CONST::convertToStr(LEG_MY_CONST::ROUTE_NP_VPA)) {abort = true;}
	if (reponse_tokens[12] != LEG_MY_CONST::convertToStr(LEG_MY_CONST::ROUTE_LENGTH_NP_VPA)) {abort = true;}
	...
	if (abort){
	   ...
	   for (std::list<std::string>::iterator it = keysValues.begin(); it != keysValues.end(); it++){
		...
		distance = LEG_MY_CONST::convertToDbl(reponse_tokens[13]);
...
std::pair<std::string, double>  LEG_GeoTraCIMobility::addEntryToEdgeBTTIndex(std::string edgeAndValue)
{
	...
	if ((value != NULL) && (std::string(value) != "")){
		if (edgeAsStr != "ND"){
			valueAsFloat = LEG_MY_CONST::convertToDbl(value);
		}
...
void LEG_GeoTraCIMobility::addRouteToEdgeBTTIndex(std::list<std::string> route)
{
	double travelTime;
	std::string edge ="";
	for (std::list<std::string>::iterator it = route.begin(); it != route.end(); it++){
...
std::string LEG_GeoTraCIMobility::sendRequestToPyServer(std::string buf)
{
//	std::string rep;
//
...
void LEG_GeoTraCIMobility::updateDisplayString() {
	ASSERT(-M_PI <= angle);
	ASSERT(angle < M_PI);
...
std::string LEG_GeoTraCIMobility::getL2Stats(cModule* macMod)
{
	std::string str;
...
//int LEG_GeoTraCIMobility::cmd_NP_ALL(){
//	int dist = 0;
//	updateCurrentSector();
...
//}
//
//void LEG_GeoTraCIMobility::rep_NP_ALL(char *data)
//{
//	std::vector<std::string> reponse_tokens = LEG_MY_CONST::tokenizeMSG(data);
//
//	// Checking if received data are correct
//	bool abort = false;
//	if (reponse_tokens[0] != LEG_MY_CONST::convertToStr(LEG_MY_CONST::RESPONSE)) {abort = true;}
...
//			edges = strtok(NULL," ");
//		}
//		currentNP.Distance_NP_VPA = LEG_MY_CONST::convertToDbl(reponse_tokens[13]);
//	}
//}

//void LEG_GeoTraCIMobility::requestingFromPyServer()
//{
////	using namespace SOCKET;
//	std::string HOST = "127.0.0.1";
//	int PORT = 19999;
...
//double LEG_GeoTraCIMobility::calculateMETD()
//{
...
//void LEG_GeoTraCIMobility::updateMETD(double lastETA_NP_VPA)
//{
//	double ETA_NP = calculateETA_NP();
//	double newMETD = 0.0;
//
