...
void TestApplLayer::handleLowerMsg( cMessage* msg )
{
    ...
    case BROADCAST_MESSAGE:
        ...
        EV << "Received a broadcast packet from host["<<m->getSrcAddr()<<"] -> sending reply" << endl;
        sendReply(m);
        break;
    case BROADCAST_REPLY_MESSAGE:
        m = static_cast<ApplPkt *>(msg);
        //coreEV << "Received reply from host["<<m->getSrcAddr()<<"]; delete msg" << endl;
        EV << "Received reply from host["<<m->getSrcAddr()<<"]; delete msg" << endl;
        delete msg;
	break;
    default:
...
{
    //ApplPkt *pkt = new ApplPkt("VPA_BROADCAST_MESSAGE", BROADCAST_MESSAGE);
    ApplPkt *pkt = new ApplPkt("BROADCAST_VPA_WMS", BROADCAST_VPA_WMS);
...
void TestApplLayer::sendReply(ApplPkt *msg)
{
    ...
    msg->setDestAddr(msg->getSrcAddr());	//get srcAddr to send back to the source.
    msg->setSrcAddr (myApplAddr() ); 		//Use myApplAddr as the source.
    msg->setKind(BROADCAST_REPLY_MESSAGE);
    msg->setName("BROADCAST_REPLY_MESSAGE");
    sendDelayedDown(msg, delay);  			// hmmm it seems that this is a way to send a delayed message.
