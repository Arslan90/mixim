...
# CELUI EST POUR  IEEE80211p
###############################################

#Data That I need to know:
$ARGC=@ARGV;
if ($ARGC != 1) { die "Usage: file\n"; }
#$nodes=$ARGV[0]; # get the number of nodes from the command line
$file=$ARGV[0]; # get the number of nodes from the command line

$nodes= 181; #number of nodes (vehicles) presented in the simulation. 
$SimTime= 600;  #600; #seconds
$intervalTX= 5; #10hz Node TX. this is the T time.
...
print "CHECKING FILE: $file \n";
	...
	@data= `/bin/grep ',RX,$i,' $file | /usr/bin/wc -l`;
	chop $data[0];
	$realRx= $data[0];
